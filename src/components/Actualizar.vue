<template>
  <div>
    <h1>Actualizar Datos en Firestore</h1>
    <table border="1">
      <tr>
        <th>Datos</th>
        <th>Correo</th>
      ...
      <tr v-for="elementos in usuarios" :key="elementos">
        ...
        <td>
          <b-button variant="warning text-white" v-on:click="callUpdate(elementos.id)">Actualizar</b-button>
        </td>
      </tr>
    </table>
  </div>
...
export default {
  ...
  methods: {
    ...
    async callUpdate(elid){
      console.log(elid);
      this.$router.push(`/formactualizar/${elid}`);
    }
  },
  mounted(){
